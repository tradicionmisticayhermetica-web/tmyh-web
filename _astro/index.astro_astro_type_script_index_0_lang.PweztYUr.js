import{a as x,f}from"./admin.BtFX5QZo.js";import"./supabase.2Xnj7Pbd.js";const r=document.getElementById("lista-posts"),c=document.getElementById("buscador"),i=document.getElementById("contador");let o=[],n="todos",d="";function s(t){return t.replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;").replace(/'/g,"&#039;")}const g={publicado:'<span class="text-[9px] uppercase tracking-widest text-gold-300 px-1.5 py-0.5 border border-gold-400/40 rounded-sm bg-gold-400/5">Publicado</span>',borrador:'<span class="text-[9px] uppercase tracking-widest text-parchment-400 px-1.5 py-0.5 border border-parchment-400/30 rounded-sm">Borrador</span>',archivado:'<span class="text-[9px] uppercase tracking-widest text-parchment-600 px-1.5 py-0.5 border border-parchment-600/20 rounded-sm opacity-60">Archivado</span>'};function l(t){if(r){if(i&&(i.textContent=`${t.length} ${t.length===1?"post":"posts"}`),t.length===0){r.innerHTML=`
        ...
        </a>`).join("")}}function p(){const t=n==="todos"?o:o.filter(a=>a.estado===n),e=d.trim().toLowerCase();if(!e)return l(t);const m=t.filter(a=>`${a.titulo} ${a.slug} ${a.extracto??""}`.toLowerCase().includes(e));l(m)}async function u(t){n=t,r&&(r.innerHTML='<div class="py-12 text-center text-parchment-400 text-sm font-serif italic">Cargando…</div>'),o=await x(200),p()}document.querySelectorAll(".filtro-btn").forEach(t=>{t.addEventListener("click",()=>{document.querySelectorAll(".filtro-btn").forEach(e=>e.classList.remove("is-active")),t.classList.add("is-active"),u(t.dataset.filtro??"todos")})});c?.addEventListener("input",()=>{d=c.value,p()});u("todos");
