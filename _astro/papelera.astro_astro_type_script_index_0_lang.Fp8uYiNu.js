import{p as h,a as w,r as v,d as y,e as E}from"./cursos-cms.CFsTa_bF.js";import"./supabase.BC3Mu_5F.js";const i=document.getElementById("lista-papelera"),l=document.getElementById("status");function o(t,e){l&&(l.textContent=t,l.classList.remove("hidden","text-gold-300","text-wine-500","text-parchment-300"),l.classList.add(e==="ok"?"text-gold-300":e==="error"?"text-wine-500":"text-parchment-300"))}function m(t="Procesando…"){window.__tmyhLoader?.show?.(t)}function p(){window.__tmyhLoader?.hide?.()}async function s(t,e,r){let a=null;try{return await Promise.race([t,new Promise((n,c)=>{a=setTimeout(()=>{c(new Error(`timeout:${r}`))},e)})])}finally{a&&clearTimeout(a)}}function L(t){return new Promise(e=>{const r=document.getElementById("dialog-confirm"),a=document.getElementById("dialog-confirm-msg"),n=document.getElementById("dialog-confirm-ok"),c=document.getElementById("dialog-confirm-cancel");if(!r||!a||!n||!c)return e(window.confirm(t));a.textContent=t,r.classList.remove("hidden"),r.classList.add("flex");const f=()=>{r.classList.add("hidden"),r.classList.remove("flex"),n.removeEventListener("click",g),c.removeEventListener("click",x)},g=()=>{f(),e(!0)},x=()=>{f(),e(!1)};n.addEventListener("click",g),c.addEventListener("click",x)})}function d(t){return t.replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;").replace(/'/g,"&#039;")}function k(t){if(!t)return"—";try{return new Date(t).toLocaleString("es-AR",{day:"2-digit",month:"short",year:"numeric",hour:"2-digit",minute:"2-digit"})}catch{return t}}async function u(){m("Cargando papelera…");let t=[];try{await s(h(),12e3,"purgar_papelera"),t=await s(w(),12e3,"listar_papelera")}catch(e){console.error("[cursos.papelera.cargar]",e),o("La papelera está tardando más de lo esperado. Reintentá en unos segundos.","error"),i&&(i.innerHTML=`
