import{c as n,f as l}from"./admin.DCvhIUVN.js";import"./supabase.2Xnj7Pbd.js";const a=document.getElementById("lista-posts"),c=document.getElementById("contador");let s=[];function r(t){return t.replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;").replace(/'/g,"&#039;")}const d={publicado:'<span class="text-[9px] uppercase tracking-widest text-gold-300 px-1.5 py-0.5 border border-gold-400/40 rounded-sm bg-gold-400/5">Publicado</span>',borrador:'<span class="text-[9px] uppercase tracking-widest text-parchment-400 px-1.5 py-0.5 border border-parchment-400/30 rounded-sm">Borrador</span>',archivado:'<span class="text-[9px] uppercase tracking-widest text-parchment-600 px-1.5 py-0.5 border border-parchment-600/20 rounded-sm opacity-60">Archivado</span>'};function p(t){if(a){if(c&&(c.textContent=`${t.length} ${t.length===1?"post":"posts"}`),t.length===0){a.innerHTML=`
        ...
        </a>`).join("")}}async function i(t){a&&(a.innerHTML='<div class="py-12 text-center text-parchment-400 text-sm font-serif italic">Cargando…</div>'),s=await n(200);const e=t==="todos"?s:s.filter(o=>o.estado===t);p(e)}document.querySelectorAll(".filtro-btn").forEach(t=>{t.addEventListener("click",()=>{document.querySelectorAll(".filtro-btn").forEach(e=>e.classList.remove("is-active")),t.classList.add("is-active"),i(t.dataset.filtro??"todos")})});i("todos");
