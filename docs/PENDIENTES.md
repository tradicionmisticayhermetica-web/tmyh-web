...
- [ ] **Bloque 3 — Edge Function `procesar-newsletter-cola`**: llama `newsletter_proximo_lote(N)`, reusa el render compartido, lo manda con Resend, reporta con `marcar_enviado` / `marcar_fallido`. Incluye rate-limit interno para no pasar de N mails/día.
